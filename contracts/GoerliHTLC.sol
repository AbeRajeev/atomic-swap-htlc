...
contract GoerliHTLC {
    ...
    function withdraw(uint256 _orderId, string memory _secret) external {
        require(
            keccak256(abi.encodePacked(_secret)) == orders[_orderId].hash,
            "You entered the wrong secret"
        );
        ...
        (bool sent, ) = orders[_orderId].receipient.call{
            value: orders[_orderId].amount
        }("");
    ...
    function refund(uint256 _orderId) external {
        ...
        require(
            orders[_orderId].completed == false,
            "The order has been completed"
        );
        require(
            block.timestamp >= orders[_orderId].lockTime,
            "Still locked. please wait"
        );
        orders[_orderId].completed = true;
        (bool sent, ) = orders[_orderId].owner.call{
            value: orders[_orderId].amount
        }("");
        require(sent, "Failed to send Ether");
    }
    ...
    function confirmOrder(uint256 _orderId) external {
        require(
            msg.sender != orders[_orderId].owner,
            "You cannot confirm your own order"
        );
        require(
            orders[_orderId].receipient == address(0),
            "Someone has already requested"
        );
